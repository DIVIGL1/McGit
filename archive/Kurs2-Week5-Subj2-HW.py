...
for classificator in classificators:
    clf_BC = classificator
    clf_BC.fit(BC_data_train, BC_target_train)
    clf_BC.predict(BC_data_test)
    BC_scoring = cross_validation.cross_val_score(clf_BC, BC_data_test, BC_target_test)
...
DG_clfs_scores = list()
for classificator in classificators:
    clf_DG = classificator
    clf_DG.fit(DG_data_train, DG_target_train)
    clf_DG.predict(DG_data_test)
    DG_scoring = cross_validation.cross_val_score(clf_DG, DG_data_test, DG_target_test)
