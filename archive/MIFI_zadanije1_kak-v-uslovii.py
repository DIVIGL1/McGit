...
f_sens = [re.split('[^a-z]', i.lower()) for i in f_sens]
f.seek(0)
f_string = f.read()
f.close()
f_string = f_string.lower() # переводим весь текст разом в нижний регистр

f_allwords = re.split('[^a-z]', f_string) # [^a-z] разбиваем текст на слова и записываем результат в список f_allwords
f_allwords = [i for i in f_allwords if i!=""] # Убираем пустые элементы
...
f_cosines=[[spatial.distance.cosine(f_matrix[0],f_matrix[i]),i] for i in range(len(f_sens))]
f_cosines.sort()
